...
class StationsDialog(Gtk.Dialog):
    ...
    def finish_initializing(self, builder, pithos):
        ...
        self.modelfilter.set_visible_func(
            lambda m, i, d: m.get_value(i, 0) and not m.get_value(i, 0).isQuickMix)
...
#        self.modelsortable.set_sort_column_id(1, Gtk.SortType.ASCENDING)
    ...
    def station_renamed(self, cellrenderertext, path, new_text):
        station = self.modelfilter[path][0]
        self.worker_run(station.rename, (new_text,),
                        context='net', message="Renaming Station...")
        self.model[self.modelfilter.convert_path_to_child_path(
            Gtk.TreePath(path))][1] = new_text
    ...
    def on_menuitem_delete(self, widget):
        ...
        if response:
            self.worker_run(station.delete, context='net',
    ...
    def add_station_cb(self, dialog, response):
        ...
        if response == 1:
            self.worker_run(
                "add_station_by_music_id", (dialog.result.musicId,),
    ...
    def station_added(self, station):
        ...
        it = self.model.insert_after(
            self.model.get_iter(1), (station, station.name))
