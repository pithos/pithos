...
class LastfmPlugin(PithosPlugin):
    ...
    def on_enable(self):
        ...
        self.song_ended_handle = self.window.connect(
            'song-ended', self.song_ended)
        self.song_changed_handle = self.window.connect(
    ...
    def song_changed(self, window, song):
        self.worker.send(self.scrobbler.report_now_playing,
    ...
    def scrobble(self, song):
        if song.duration > 30 and (song.position > 240 or song.position > song.duration / 2):
            ...
            self.worker.send(self.scrobbler.scrobble, (song.artist, song.title, int(
                song.start_time), source, mode, song.duration, song.album))
