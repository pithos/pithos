...
class PithosNotificationIcon(PithosPlugin):
    ...
    def on_enable(self):
        ...
        if indicator_capable:
            ...
            self.statusicon = Gtk.StatusIcon.new_from_file(
    ...
    def build_context_menu(self):
        ...
        def button(text, action, icon=None):
            if icon == 'check':
                ...
                item.set_image(
                    Gtk.Image.new_from_stock(icon, Gtk.IconSize.MENU))
            else:
        ...
        if indicator_capable:
            ...
            self.visible_check = button(
                "Show Pithos", self._toggle_visible, 'check')

        self.playpausebtn = button(
            "Pause", self.window.playpause, Gtk.STOCK_MEDIA_PAUSE)
        ...
        if indicator_capable:
            ...
            self.statusicon.connect(
                'scroll-event', lambda _, steps: self.scroll(steps))
    ...
    def play_state_changed(self, window, playing):
        ...
        if not playing:
            ...
            button.set_image(
                Gtk.Image.new_from_stock(Gtk.STOCK_MEDIA_PLAY, Gtk.IconSize.MENU))

        else:
            button.set_label("Pause")
            button.set_image(
                Gtk.Image.new_from_stock(Gtk.STOCK_MEDIA_PAUSE, Gtk.IconSize.MENU))
